...
`showsBackgroundLocationIndicator` maps directly to Apple's
[`CLLocationManager.showsBackgroundLocationIndicator`](https://developer.apple.com/documentation/corelocation/cllocationmanager/showsbackgroundlocationindicator). The name is counter‑intuitive — it is an **opt‑in to *show* the indicator, not a switch to hide it**:

| Value | Meaning |
|---|---|
| `true` | **Show** the blue status‑bar pill / Dynamic Island indicator while the app uses location in the background. |
...
3. Avoid unnecessary continuous background location. **Geofence‑only mode uses native region monitoring (no continuous GPS), so it shows no indicator** — `startGeofences()` does not run continuous updates in standard mode.
