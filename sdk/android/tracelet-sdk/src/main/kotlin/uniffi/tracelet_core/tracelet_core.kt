...
external fun uniffi_tracelet_core_fn_method_impactdetector_on_impact_window(`ptr`: Long,`peakG`: Double,`speedBeforeMps`: Double,`gyroPeakDps`: Double,`isOnFoot`: Byte,`latitude`: Double,`longitude`: Double,`nowMs`: Long,uniffi_out_err: UniffiRustCallStatus, 
): RustBuffer.ByValue
...
private fun uniffiCheckApiChecksums(lib: IntegrityCheckingUniffiLib) {
    ...
    if (lib.uniffi_tracelet_core_checksum_method_impactdetector_on_impact_window() != 64785.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
...
public interface ImpactDetectorInterface {
     ...
     */
    fun `onImpactWindow`(`peakG`: kotlin.Double, `speedBeforeMps`: kotlin.Double, `gyroPeakDps`: kotlin.Double, `isOnFoot`: kotlin.Boolean, `latitude`: kotlin.Double, `longitude`: kotlin.Double, `nowMs`: kotlin.Long): ImpactEvent?
...
open class ImpactDetector: Disposable, AutoCloseable, ImpactDetectorInterface
{
     ...
     * `potential_*` event when an impact is detected (and registers it for
     * confirmation), else `None`.
     */override fun `onImpactWindow`(`peakG`: kotlin.Double, `speedBeforeMps`: kotlin.Double, `gyroPeakDps`: kotlin.Double, `isOnFoot`: kotlin.Boolean, `latitude`: kotlin.Double, `longitude`: kotlin.Double, `nowMs`: kotlin.Long): ImpactEvent? {
            return FfiConverterOptionalTypeImpactEvent.lift(
    callWithHandle {
    uniffiRustCall() { _status ->
    UniffiLib.uniffi_tracelet_core_fn_method_impactdetector_on_impact_window(
        it,
        FfiConverterDouble.lower(`peakG`),FfiConverterDouble.lower(`speedBeforeMps`),FfiConverterDouble.lower(`gyroPeakDps`),FfiConverterBoolean.lower(`isOnFoot`),FfiConverterDouble.lower(`latitude`),FfiConverterDouble.lower(`longitude`),FfiConverterLong.lower(`nowMs`),_status)
}
    }
    )
    }
