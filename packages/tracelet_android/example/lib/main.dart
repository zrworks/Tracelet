...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _start() async {
    try {
      ...
      if (authStatus < 2) {
        _addLog(
          'WARN',
          'Location permission denied (status=$authStatus). Cannot start.',
        );
        return;
      }
  ...
  // ── One-Shot Location (no foreground service) ───────────────────────────
  ...
  /// and do not want a persistent notification.
  ///
  /// **Warning:** On Android 8+ (API 26+), background location may be
  /// restricted without a foreground service.
  Future<void> _initNoForeground() async {
    try {
      ...
      final state = await tl.Tracelet.ready(
        tl.Config(
          ...
          app: const tl.AppConfig(
            ...
            // Disable the foreground service — no notification shown.
      ...
      _addLog(
        'READY',
        'Initialized WITHOUT foreground service  enabled=${state.enabled}',
      );
    } catch (e) {
