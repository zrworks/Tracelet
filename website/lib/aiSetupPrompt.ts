...
5. Do you have a backend to sync locations to? If yes: what is the URL, and what auth headers does it need? Should syncing be batched, Wi-Fi-only, or real-time?
...
4. Wire up the full initialization in the right place in my app: register the headless task before \`runApp\` with \`@pragma('vm:entry-point')\`, request notification/motion/location authorization in the correct order, check \`getSettingsHealth()\` and offer \`showPowerManager()\` on aggressive OEMs, configure the Android foreground service notification text to match my app's tone, set \`stopOnTerminate\`/\`startOnBoot\` from my answers, attach sync \`url\`/\`headers\` if I have a backend, then call \`Tracelet.ready(config)\` and \`Tracelet.start()\` (or explain where to trigger start if tracking shouldn't begin at launch).
5. If I asked for enterprise features, set up \`SecurityConfig\` (generate the encryption key with \`Tracelet.generateEncryptionKey()\` and store it in secure storage), privacy zones, or SSL pinning per the enterprise docs.
6. Add event listeners (\`onLocation\`, \`onMotionChange\`, geofence events if enabled) with sensible placeholder handlers I can extend.
...
- Anything I must do manually (App Store background-location review notes, backend endpoint expectations, testing on a real device).
