...
Add background location and foreground service permissions (See the full XML snippet in the [Android Guide](/android)).
...
void main() async {
  ...
  // 3. Request permissions required for background tracking
  // Tracelet handles Location, but you must manually handle OS-level permissions
  if (Platform.isAndroid) {
    await Permission.notification.request(); // Required for Android 13+ Foreground Service
    await Permission.activityRecognition.request(); // Required for Motion Detection
  }
