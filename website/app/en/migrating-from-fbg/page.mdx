...
import DynamicPubspec from '../../../components/DynamicPubspec'

> 💡 This guide always reflects the current release.
...
- **`security`** → `SecurityConfig` — 🆕 At-rest database encryption (`encryptDatabase`)
- **`attestation`** → `AttestationConfig` — 🆕 Device attestation / integrity checks
...
triggerActivities                      → motion.activityTypes (Set<ActivityType>)
```

**🆕 Tracelet-exclusive MotionConfig fields:**

- `motion.motionDetectionMode` — `.accelerometer` / `.speed` / `.smart` — how movement is detected (smart = accelerometer **and** GPS speed; recommended)
...
- `motion.speedMovingThreshold` / `motion.speedStationaryDelay` / `motion.speedWakeConfirmCount` — speed-mode tuning (used by `.speed` / `.smart`)
- `motion.stationaryTrackingMode` / `motion.stationaryPeriodicInterval` / `motion.stationaryPeriodicAccuracy` — what to do once stationary (periodic fixes vs geofences)
...
- **Web support** — The same Dart API runs in the browser for foreground (in-tab) tracking. Background tracking and several native features aren't available — see [Web Support](/en/reference/web-support).
