/// iOS implementation of the Tracelet background geolocation plugin.
library;

import 'package:tracelet_platform_interface/tracelet_platform_interface.dart';

/// The iOS implementation of [TraceletPlatform].
///
/// This class registers itself as the platform implementation by calling
/// [TraceletPlatform.instance] with a [PigeonTracelet] instance
/// during [registerWith].
class TraceletIos extends PigeonTracelet {
  /// Registers this class as the default instance of [TraceletPlatform].
  static void registerWith() {
    TraceletPlatform.instance = TraceletIos();
  }
}
