...
void main() {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // LocationProcessor with adaptive mode
  // ═══════════════════════════════════════════════════════════════════════

  group('LocationProcessor with enableAdaptiveMode', () {
    test('adaptive mode uses activity-based distance filter', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );

      // Accept the first location (always passes).
      ...
      // Second location 100m away — still activity uses 500m filter,
      ...
      expect(second.reason, 'DISTANCE_FILTER');
    });

    test('adaptive mode with inVehicle accepts short distances', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );
      ...
      // Second location ~15m away — inVehicle uses 10m filter,
      // so 15m should be accepted.
      final second = processor.process(
        latitude: 37.000135,
        longitude: -122.0,
        accuracy: 10.0,
    ...
    test('adaptive mode still applies accuracy and speed filters', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      ...
      // Second location far enough but poor accuracy.
    ...
    test('adaptive mode with low battery increases distance filter', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );
      ...
      processor.process(
        ...
        adaptiveContext: const AdaptiveContext(
          activityType: ActivityType.walking,
          activityConfidence: ActivityConfidence.high,
          batteryLevel: 0.05, // Critical battery
        ),
      );

      // Second location ~100m away.
      // Walking profile = 50m × 5.0 (battery) = 250m → reject 100m.
      final result = processor.process(
        latitude: 37.0009,
        longitude: -122.0,
        accuracy: 10.0,
    ...
    test('copyWith preserves enableAdaptiveMode', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );

      final copy = processor.copyWith(distanceFilter: 20.0);
      expect(copy.enableAdaptiveMode, isTrue);
      expect(copy.distanceFilter, 20.0);
    });

    test('copyWith can change enableAdaptiveMode', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );

      final copy = processor.copyWith(enableAdaptiveMode: false);
      expect(copy.enableAdaptiveMode, isFalse);
    });

    test('without adaptiveContext falls back to speed-based', () {
      final processor = LocationProcessor(
        distanceFilter: 10.0,
        enableAdaptiveMode: true,
      );
      ...
      // 20m away, speed=50m/s → speedFactor=clamp(50/10,1,10)=5.0
      // effectiveDistance=50m → reject 20m.
      final result = processor.process(
        latitude: 37.00018,
        longitude: -122.0,
        accuracy: 10.0,
