...
If your app doesn't need a specific feature (for example, you don't use Periodic Mode, or you don't need background location), you can forcefully remove the permission using Android's manifest merger `tools:node="remove"` directive in your **app-level** `AndroidManifest.xml` (`android/app/src/main/AndroidManifest.xml`).
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <!-- Remove Activity Recognition: Falls back to basic accelerometer-only motion detection -->
    <uses-permission android:name="android.permission.ACTIVITY_RECOGNITION" tools:node="remove" />
    
    <!-- Remove Background Location: Stops tracking when the app is completely backgrounded on Android 10+ -->
    <uses-permission android:name="android.permission.ACCESS_BACKGROUND_LOCATION" tools:node="remove" />

    <!-- Remove Exact Alarms: Periodic mode will use battery-friendly inexact WorkManager instead -->
...
<Callout type="info" emoji="🔔">
  **Android 13+ Note:** You must request the `POST_NOTIFICATIONS` permission before starting the service, otherwise the notification will be silently suppressed by the OS. See the [Flutter SDK: Permissions](/flutter#scenario-2-the-onboarding-flow) page for exactly how to request this from your Dart code.
</Callout>
...
For a detailed example of how to implement this in your Flutter UI, see the [Diagnostic Tools & Power Management](/tools#oem-power-manager-settings-health) page.
