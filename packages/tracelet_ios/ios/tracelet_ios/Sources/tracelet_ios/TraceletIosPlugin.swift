...
@_silgen_name("frbgen_tracelet_platform_interface_rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerTripManagerDart") func dummy_arc_dec_5()
    ...
    /// This dummy method is required to prevent Xcode's Dead Code Stripping (in Release builds)
    /// from removing the C-symbols generated by flutter_rust_bridge.
    /// Because Dart FFI calls these functions at runtime, if they are stripped from the iOS
    /// plugin binary, the app will crash with a "Failed to lookup symbol" error.
    /// By referencing them in this public method, we force the linker to preserve them.
