...
14. Permissions — I want to request **only** the permissions my use case actually needs, and I do NOT want unused permissions left in my iOS Info.plist or Android manifest. Using my earlier answers, tell me the exact permission set you'll request and ask me to confirm anything you're unsure about:
   ...
   - Notifications — needed on Android for the persistent foreground-service notification, and for any SOS/impact alert I enable.
...
4. Wire up the full initialization in the right place in my app: register the headless task before \`runApp\` with \`@pragma('vm:entry-point')\`, request only the notification/motion/location authorizations my chosen features require, in the correct order, check \`getSettingsHealth()\` and offer \`showPowerManager()\` on aggressive OEMs, configure the Android foreground service notification text to match my app's tone, set \`stopOnTerminate\`/\`startOnBoot\` from my answers, then call \`Tracelet.ready(config)\` and \`Tracelet.start()\` (or explain where to trigger start if tracking shouldn't begin at launch).
